...
func _ready() -> void:
	...
	# Touch rows only where there is a touchscreen to point them at - a desktop
	# gets a toggle that toggles nothing otherwise. Built by the overlay's own
	# helper so this screen and the in-match panel show the identical rows.
	if MobileControls.device_supports_touch():
		MobileControls.add_settings_rows($Panel/VBox, $Panel/VBox/MasterRow/MasterLabel)
