...
## The touch overlay lives in its own scene (ui/mobile/mobile_controls.tscn)
## and drives the game purely by pressing the same input actions the keyboard
## does, so it needs nothing from this script beyond a place to sit. It goes
## under HUD rather than in this scene's tree so it draws over the world at the
## HUD's layer and comes and goes with the HUD.
##
## Only instanced where there is a touchscreen to use it; a desktop never sees
## it, and never sees the two settings rows that would switch it either. The
## on/off setting itself is the overlay's business - see _apply_settings there -
## so a toggle flipped from the in-match panel lands immediately. Pass "--touch"
## on the command line to force all of this on for mouse testing.
const MOBILE_CONTROLS_SCENE: PackedScene = preload("res://ui/mobile/mobile_controls.tscn")

## Breathing room between the top of the highest touch button and the bottom
## of the event feed once the feed has been moved up out of its way.
const FEED_CLEARANCE_ABOVE_TOUCH := 8.0

@onready var event_feed: Control = $HUD/EventFeed

func _setup_mobile_controls() -> void:
	if not MobileControls.device_supports_touch():
		return
	var controls: MobileControls = MOBILE_CONTROLS_SCENE.instantiate()
	controls.name = "MobileControls"
	$HUD.add_child(controls)

	# The feed and the touch buttons both live in the bottom-right corner. Rather
	# than pin the buttons somewhere the feed is not, the feed moves: it is read
	# occasionally, the buttons are held constantly. Both are bottom-anchored, so
	# their offsets share the screen's bottom edge as a reference and compare
	# directly - and reading the buttons' own offsets means the feed follows
	# wherever they end up, instead of a number here going stale the next time
	# the overlay is rearranged.
	var feed_height := event_feed.offset_bottom - event_feed.offset_top
	event_feed.offset_bottom = controls.highest_control_offset() - FEED_CLEARANCE_ABOVE_TOUCH
	event_feed.offset_top = event_feed.offset_bottom - feed_height

	# Same two rows the main Settings screen gets, from the same builder. Styled
	# after the Master row's name label so they read as part of the same panel.
	MobileControls.add_settings_rows(
		settings_popup.get_node("VBox"),
		settings_popup.get_node("VBox/MasterRow/MasterLabel"),
		leave_game_button)
